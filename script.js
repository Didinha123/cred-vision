...
var APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycby4FmCiBJjIMDlhkAq83M0w735DIENjybhcHnGCMOm5RlDKDQdXdDhhXOD8OOm0FPw1/exec";
...
function renderCadastro() {
  ...
  var rows = clients.map(function(c) {
    ...
    var fotos = FOTO_SLOTS.filter(function(s){ return c.fotos && c.fotos[s.id]; });
    var fotoHtml = fotos.length
      ? fotos.map(function(s){
          return "<img src='" + c.fotos[s.id] + "' title='" + s.label + "' style='width:30px;height:30px;object-fit:cover;border-radius:4px;border:1px solid var(--border);cursor:pointer;' onclick='event.stopPropagation();viewClient(\""+c.id+"\")' />";
        }).join("")
      : "<span style='color:var(--text-muted);font-size:11px;'>—</span>";
    return "<tr onclick='viewClient(\""+c.id+"\")' style='cursor:pointer;border-bottom:1px solid var(--border);transition:background .15s;' onmouseover=\"this.style.background='var(--bg3)'\" onmouseout=\"this.style.background=''\">" +
      "<td style='padding:8px 6px;font-weight:600;color:var(--text);'>" + c.nome + "</td>" +
      "<td style='padding:8px 6px;color:var(--text-muted);font-size:12px;'>" + (c.tel || "—") + "</td>" +
      "<td style='padding:8px 6px;'><div style='display:flex;gap:4px;flex-wrap:wrap;align-items:center;'>" + fotoHtml + "</div></td>" +
      "<td style='padding:8px 6px;font-size:12px;color:var(--text-muted);text-align:center;'>" + (loanCount ? loanCount : "—") + "</td>" +
      "<td style='padding:8px 6px;font-size:11px;color:var(--text-muted);'>" + fmtDate(c.createdAt) + "</td>" +
      "<td style='padding:8px 6px;white-space:nowrap;'>" +
        "<button class='btn btn-sm' style='background:var(--bg3);border:1px solid var(--border-gold);color:var(--gold);padding:2px 8px;font-size:11px;' onclick='event.stopPropagation();viewClient(\""+c.id+"\")'>🪪 Ver</button> " +
        "<button class='btn btn-sm' style='background:var(--bg3);border:1px solid var(--border);color:var(--text-muted);padding:2px 8px;font-size:11px;' onclick='event.stopPropagation();editClient(\""+c.id+"\")'>✏️</button>" +
      "</td>" +
    "</tr>";
  }).join("");
  list.innerHTML =
    "<table style='width:100%;border-collapse:collapse;font-size:13px;'>" +
      "<thead><tr style='border-bottom:2px solid var(--border-gold);'>" +
        "<th style='text-align:left;padding:8px 6px;color:var(--gold);font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;'>Nome</th>" +
        "<th style='text-align:left;padding:8px 6px;color:var(--gold);font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;'>Telefone</th>" +
        "<th style='text-align:left;padding:8px 6px;color:var(--gold);font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;'>Fotos</th>" +
        "<th style='text-align:center;padding:8px 6px;color:var(--gold);font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;'>Emp.</th>" +
        "<th style='text-align:left;padding:8px 6px;color:var(--gold);font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;'>Cadastro</th>" +
